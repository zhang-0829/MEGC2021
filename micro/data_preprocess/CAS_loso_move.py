...
def optical_flow(img1,img2):
    img1 = cv2.cvtColor(img1,cv2.COLOR_BGR2GRAY) #灰度
...
def read_videoFrames(path):
    ...
    sorted_frames = sorted(frames, key=lambda x: re.findall("\d+", x)[0])
...
if __name__ == '__main__':
    ...
    cas_path = './longVideoFaceCropped_{0}'

    type_names = ['micro','macro']
    ...
    for subject_id in subject_ids:
        ...
        for type_id, type in enumerate(types):
            ...
            for data_path in data_paths:
                subject_path = os.path.join(ori_path.format(type),data_path)
                print('个人路径：')
                print(subject_path)
                now_id = data_path.split(char)[0]
                print(now_id)
                print(subject_id)
                if  now_id != subject_id:
                    for exp_path in os.listdir(subject_path):
                        print(exp_path)
                        l , r , m , f= getData(os.path.join(subject_path,exp_path))
                        datas[0].append(l)
                        datas[1].append(r)
                        datas[2].append(m)
                        datas[3].append(f)
