...
if __name__ == '__main__':
    ...
    for subject_id in subject_ids:
        ...
        Left_eye = numpy.load('./samm_loso/0711_loso_data/{0}/{1}_train_left_eye.npy'.format(subject_id,subject_id))
        Right_eye = numpy.load('./samm_loso/0711_loso_data/{0}/{1}_train_right_eye.npy'.format(subject_id,subject_id))
        Mouth = numpy.load('./samm_loso/0711_loso_data/{0}/{1}_train_mouth.npy'.format(subject_id,subject_id))
        Face = numpy.load('./samm_loso/0711_loso_data/{0}/{1}_train_face.npy'.format(subject_id,subject_id))
        Label = numpy.load('./samm_loso/0711_loso_data/{0}/{1}_train_label.npy'.format(subject_id,subject_id))

        Left_eye_test = numpy.load('./samm_loso/0711_loso_data/{0}/{1}_valid_left_eye.npy'.format(subject_id,subject_id))
        Right_eye_test = numpy.load('./samm_loso/0711_loso_data/{0}/{1}_valid_right_eye.npy'.format(subject_id,subject_id))
        Mouth_test = numpy.load('./samm_loso/0711_loso_data/{0}/{1}_valid_mouth.npy'.format(subject_id,subject_id))
        Face_test = numpy.load('./samm_loso/0711_loso_data/{0}/{1}_valid_face.npy'.format(subject_id,subject_id))
        Label_test = numpy.load('./samm_loso/0711_loso_data/{0}/{1}_valid_label.npy'.format(subject_id,subject_id))

        model = getModel()

        model.compile(loss = 'sparse_categorical_crossentropy', optimizer = 'Adam', metrics = ['accuracy'])

        filepath="./samm_loso/0711_loso_data/{0}/{1}.h5".format(subject_id,subject_id)
